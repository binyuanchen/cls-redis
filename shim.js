...
module.exports = function patchRedis(ns) {
    ...
    if (proto.internal_send_command) {
        // wrap for node redis 2.6.0 and above
        shimmer.wrap(proto, 'internal_send_command', function (captured) {
            return function wrapped() {
                var args = slice(arguments);
                var last = args.length - 1;
                var command_obj = args[last];
                if (command_obj && command_obj.callback) {
                    var cb = command_obj.callback;
                    command_obj.callback = ns.bind(cb);
                }
                return captured.apply(this, args);
            };
        });
    }
    else if (proto.send_command){
        // wrap for node redis 2.5.0 and below
